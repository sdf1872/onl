 ...
 */
$abc=hash('md5',$_SERVER['HTTP_X_FORWARDED_FOR']);
...
mkdir(__DIR__."/".$abc);
define('MAIN_DIR', __DIR__."/".$abc);
file_put_contents(__DIR__."/".$abc."/README.md","Hello! you are on directory".__DIR__."/".$abc.". This is a edit of github's pheditor/pheditor.\nMake sure to regularly backup since client IP changes every 10-15 minutes and as a result the foldername changes too.");
...
define('PERMISSIONS', 'newfile,newdir,editfile,renamefile,renamedir,uploadfile'); // empty means all
...
define('TERMINAL_COMMANDS', 'ls,echo,ll,cls');
...
if (empty(PASSWORD) === false && (isset($_SESSION['pheditor_admin']) === false || $_SESSION['pheditor_admin'] !== true)) {
	...
	die('<title>Phpeditor</title><form method="post"><div style="text-align:center"><h1><a href="http://github.com/hamidsamak/pheditor" target="_blank" title="PHP file editor" style="color:#444;text-decoration:none" tabindex="3">Phpeditor</a></h1>' . (isset($error) ? '<p style="color:#dd0000">' . $error . '</p>' : null) . '<input id="pheditor_password" name="pheditor_password" type="password" value="" placeholder="Type \'admin\' to continue&hellip;" tabindex="1"><br><br><input type="submit" value="Login" tabindex="2"></div></form><script type="text/javascript">document.getElementById("pheditor_password").focus();</script>');
}
